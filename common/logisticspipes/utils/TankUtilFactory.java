...
public class TankUtilFactory {
	...
	public ITankUtil getTankUtilForTE(TileEntity tile, EnumFacing entityFace) {
		if (SimpleServiceLocator.specialTankHandler.hasHandlerFor(tile)) {
			...
			if (handler instanceof ISpecialTankAccessHandler) {
				if (tile.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, entityFace)) {
					IFluidHandler fluidHandler = tile.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, entityFace);
		...
		if (tile != null && tile.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, entityFace)) {
			IFluidHandler fluidHandler = tile.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, entityFace);
			if (fluidHandler != null)
