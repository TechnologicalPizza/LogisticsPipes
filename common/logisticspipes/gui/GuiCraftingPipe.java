...
public class GuiCraftingPipe extends ModuleBaseGui {
	...
	public GuiCraftingPipe(EntityPlayer player, IInventory dummyInventory, ModuleCrafter module, boolean isAdvancedSat, int liquidCrafter, int[] amount, boolean hasByproductExtractor, boolean isFuzzy, int cleanupSize, boolean cleanupExclude) {
		...
		for (int i = 0; i < liquidCrafter; i++) {
			...
			fluidSlotIDs[i] = extensionControllerLeft.registerControlledSlot(dummy.addFluidSlot(i, module.getFluidInventory(), liquidLeft + 11, 24));
		}
		...
		for (int y = 0; y < cleanupSize; y++) {
			for (int x = 0; x < 3; x++) {
				cleanupSlotIDs[y * 3 + x] = extensionControllerLeft.registerControlledSlot(dummy.addDummySlot(y * 3 + x, module.getCleanupInventory(), x * 18 - 57, y * 18 + 13));
			}
		}
	...
	public void initGui() {
		...
		if (!isAdvancedSat) {
			...
			if (liquidCrafter != 0) {
				extension.registerButton(extensionControllerLeft.registerControlledButton(addButton(normalButtonArray[5] = new SmallGuiButton(22, guiLeft - (liquidCrafter * 40) / 2 - 18, guiTop + 158, 37, 10, StringUtils.translate(PREFIX + "Select")))));
			}
		} else {
			for (int i = 0; i < 9; i++) {
		...
		for (int i = 0; i < liquidCrafter; i++) {
			...
			if (isAdvancedSat) {
				extension.registerButton(extensionControllerLeft.registerControlledButton(addButton(liquidGuiParts[i][8] = new SmallGuiButton(100 + 10 * i + 8, liquidLeft + 2, guiTop + 160, 37, 10, StringUtils.translate(PREFIX + "Select")))));
		...
		if (cleanupSize > 0) {
			...
			cleanupExtension.registerButton(extensionControllerLeft.registerControlledButton(addButton(cleanupModeButton = new SmallGuiButton(24, guiLeft - 56, guiTop + 18 + (18 * cleanupSize), 50, 10, StringUtils.translate(GuiCraftingPipe.PREFIX + (_pipe.cleanupModeIsExclude ? "Exclude" : "Include"))))));
			cleanupExtension.registerButton(extensionControllerLeft.registerControlledButton(addButton(new SmallGuiButton(25, guiLeft - 56, guiTop + 32 + (18 * cleanupSize), 50, 10, StringUtils.translate(GuiCraftingPipe.PREFIX + "Import")))));
